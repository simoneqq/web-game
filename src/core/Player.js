...
export class Player {
  ...
  checkCollision(pos) {
    ...
    const centerPos = new THREE.Vector3(pos.x, footLevel + colliderHeight / 2, pos.z);
  ...
  update(delta) {
    ...
    let speed = 100.0;
    this.currentHeight = keys.crouch ? this.crouchHeight : this.standingHeight;
    ...
    if (keys.forward || keys.backward) this.velocity.z -= this.direction.z * speed * delta;
    if (keys.left || keys.right) this.velocity.x -= this.direction.x * speed * delta;
    ...
    // TEST OSI Z
    const posAfterX = obj.position.clone();
    this.controls.moveForward(-this.velocity.z * delta);
    if (this.checkCollision(obj.position)) {
        obj.position.z = posAfterX.z;
    }
    ...
    this.playerBB.setFromCenterAndSize(
        new THREE.Vector3(obj.position.x, footLevel + (this.currentHeight + 0.2)/2, obj.position.z),
    ...
    if (!onObject && Math.abs(obj.position.x) < halfMap && Math.abs(obj.position.z) < halfMap) {
